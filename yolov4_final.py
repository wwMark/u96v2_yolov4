...
from tensorflow.keras.layers import Conv2D, Input, BatchNormalization, LeakyReLU, ZeroPadding2D, UpSampling2D, MaxPool2D, Activation, Flatten, Dense
...
def make_yolov4_model():
    ...
    x = _conv_block(input_image, [{'filter': 32, 'kernel': 3, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 0}])
    ...
    x = _conv_block(x, [{'filter':  64, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 2}])
    layer_2 = x
    ...
    x = _conv_block(x, [{'filter': 64, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 4},                  
                        {'filter': 32, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 5},
                        {'filter': 64, 'kernel': 3, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 6}],
                   skip = True) 
    
    ## Layer 8 [convolutional]
    x = _conv_block(x, [{'filter':  64, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 8}])
    layer_8 = x
    ...
    x = _conv_block(x, [{'filter':  64, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 10},
                        {'filter': 128, 'kernel': 3, 'stride': 2, 'bnorm': True, 'activ': 1, 'layer_idx': 11}])
    layer_11 = x
    # ---------- end 1*Conv+conv+residual 304*304
   
    # ---------- begin 2*Conv+conv+residual 152*152      
    # Layer 12 [convolutional]
    x = _conv_block(x, [{'filter':  64, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 12}])
    layer_12 = x
    ...
    x = _conv_block(x, [{'filter':  64, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 14},                    
                        {'filter':  64, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 15},
                        {'filter':  64, 'kernel': 3, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 16}],
                   skip = True)
    
    ## Layer 18 => 19 [convolutional] 
    x = _conv_block(x, [{'filter':  64, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 18},
                        {'filter':  64, 'kernel': 3, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 19}],
                   skip = True)
    
    ## Layer 21 [convolutional] 
    x = _conv_block(x, [{'filter':  64, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 21}]) 
    layer_21 = x
    ...
    x = _conv_block(x, [{'filter':  128, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 23},
                        {'filter':  256, 'kernel': 3, 'stride': 2, 'bnorm': True, 'activ': 1, 'layer_idx': 24}])
    layer_24 = x
    # ---------- end 2*Conv+conv+residual 152*152        
    
    # ---------- begin 8*Conv+conv+residual 76*76    
    ## Layer  25 [convolutional] 
    x = _conv_block(x, [{'filter':  128, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 25}])
    layer_25 = x
    ...
    x = _conv_block(x, [{'filter':  128, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 27},                       
                        {'filter':  128, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 28},
                        {'filter':  128, 'kernel': 3, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 29}],
                   skip = True)
    
    
    ## Layer 31 => 50 [convolutional] and [shortcut] for layers 33, 36, 39, 42, 45, 48, 51
    for i in range(7):
        x = _conv_block(x, [{'filter': 128, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 31+(i*3)},
                            {'filter': 128, 'kernel': 3, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 32+(i*3)}],
                       skip = True)
  
    ## Layer 52 [convolutional]
    x = _conv_block(x, [{'filter':  128, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 52}])
    layer_52 = x
    ...
    x = _conv_block(x, [{'filter':  256, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 54}])
    # ---------- initial output 76*76 
    layer_54 = x
    
    ## Layer 55 [convolutional]
    x = _conv_block(x, [{'filter':  512, 'kernel': 3, 'stride': 2, 'bnorm': True, 'activ': 1, 'layer_idx': 55}])
    layer_55 = x
    # ---------- end 8*Conv+conv+residual 76*76        
    
    # ---------- begin 8*Conv+conv+residual 38*38  
    ## Layer 56 [convolutional]
    x = _conv_block(x, [{'filter':  256, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 56}])
    layer_56 = x
    ...
    x = _conv_block(x, [{'filter':  256, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 58},
                        {'filter':  256, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 59},
                        {'filter':  256, 'kernel': 3, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 60}],
                   skip = True)     
    
    ## Layer 62 => 81 [convolutional] and [shortcut] for layers 64, 67, 70, 73, 76, 79, 82
    for i in range(7):
        x = _conv_block(x, [{'filter': 256, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 62+(i*3)},
                            {'filter': 256, 'kernel': 3, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 63+(i*3)}],
                       skip = True)

    ## Layer 83 [convolutional]
    x = _conv_block(x, [{'filter':  256, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 83}])
    layer_83 = x
    ...
    x = _conv_block(x, [{'filter':   512, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 85}])
    layer_85 = x
    # ---------- initial output 38*38 
   
    
    ## Layer 86 [convolutional]
    x = _conv_block(x, [{'filter':  1024, 'kernel': 3, 'stride': 2, 'bnorm': True, 'activ': 1, 'layer_idx': 86}])
    layer_86 = x
    
    ## Layer 87 [convolutional]
    x = _conv_block(x, [{'filter':  512, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 87}])
    layer_87 = x
    ...
    x = _conv_block(x, [{'filter':  512, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 89},                         
                        {'filter':  512, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 90},
                        {'filter':  512, 'kernel': 3, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 91}],
                   skip = True) 
    
    
    ## Layer 93 => 100 [convolutional] and [shortcut] for layers 95, 98, 101
    for i in range(3):
        x = _conv_block(x, [{'filter': 512, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 93+(i*3)},
                            {'filter': 512, 'kernel': 3, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 94+(i*3)}],
                       skip = True)  
    
    # ---------- end 4*Conv+conv+residual 19*19 
    
    ## Layer 102 [convolutional]
    x = _conv_block(x, [{'filter':  512, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 102}])  
    layer_102 = x
    ...
    x = _conv_block(x, [{'filter':  1024, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 104},
    ...
    ## Layer 110 [maxpool]
    x =MaxPool2D(pool_size=(6, 6), strides=1, padding='same', name = 'layer_110')(x)    
    layer_110 = x
    ...
    ## Layer 112 [maxpool]
    x =MaxPool2D(pool_size=(8, 8), strides=1, padding='same', name = 'layer_112')(x) 
    layer_112 = x
    ...
    x = _conv_block(x, [{'filter':  255, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ':  1, 'layer_idx': 138}])   
    ...
    x = _conv_block(x, [{'filter':  512, 'kernel': 3, 'stride': 1, 'bnorm': True, 'activ':  1, 'layer_idx': 148},
                        {'filter':  255, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ':  1, 'layer_idx': 149}])
...
# ---------------------------------- end head output 38*38 ---------------------------------------
    ...
    # -- beging last Convulationnal 3*3 and 1*1 for 19*19                   
                        ...
                        {'filter':   255, 'kernel': 1, 'stride': 1, 'bnorm': True, 'activ':    1, 'layer_idx': 160}])  
    ...
    # x = _conv_block(x, [{'filter': 80, 'kernel': 8, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 444},
    #                     {'filter': 80, 'kernel': 8, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 445},
    #                     {'filter': 80, 'kernel': 5, 'stride': 1, 'bnorm': True, 'activ': 1, 'layer_idx': 446}])
    x = Conv2D(80, 8, strides=(1, 1), padding='valid', name='final_conv_444')(x)
    x = Conv2D(80, 8, strides=(1, 1), padding='valid', name='final_conv_445')(x)
    x = Conv2D(80, 5, strides=(1, 1), padding='valid', name='final_conv_446')(x)
    model = Model(input_image, x, name='Yolo_v4')
    return model
